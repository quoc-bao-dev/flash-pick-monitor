...
export function MetricCard({
  ...
  const isPositive = invertTrend ? trendDir === 'down' : trendDir === 'up';

  const trendColor = trendDir === 'neutral' ? 'text-zinc-500' : isPositive ? 'text-emerald-400' : 'text-red-400';
  ...
  return (
    <div
      ...
      {...props}
    >
      <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold mb-1.5">{label}</p>
      <p className={cn('font-headline font-bold text-on-surface', size === 'sm' ? 'text-xl' : 'text-2xl')}>{value}</p>
